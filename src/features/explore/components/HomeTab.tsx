...
export function HomeTab({
  ...
  useEffect(() => {
    ...
    fetchStationData()
  }, [uniqueStations, setIsLoadingStationData, setStationData, stationData])
  ...
  return (
    <div className="h-full flex flex-col bg-[var(--valor-bg)] overflow-hidden">
      {/* Stations Nearby Banner */}
      <div className="flex-shrink-0 bg-gradient-to-r from-[var(--valor-green)] to-[var(--valor-green-dark)] px-6 py-4 text-white shadow-sm" style={{ paddingLeft: 'calc(env(safe-area-inset-left, 0px) + 1.5rem)', paddingRight: 'calc(env(safe-area-inset-right, 0px) + 1.5rem)' }}>
        <div className="flex items-center justify-between gap-4">
          <div>
            <p className="text-[11px] opacity-90 uppercase tracking-[0.08em]">{t('homeTab.stationsNearby')}</p>
      ...
      <div className="flex-shrink-0 bg-white/90 backdrop-blur-sm py-3 border-b border-black/5" style={{ paddingLeft: 'calc(env(safe-area-inset-left, 0px) + 1.25rem)', paddingRight: 'calc(env(safe-area-inset-right, 0px) + 1.25rem)' }}>
      ...
      {/* Gas Station Cards */}
      <div className="flex-1 overflow-y-auto" style={{ overscrollBehavior: 'contain', padding: 'var(--spacing-md) var(--spacing-md) var(--spacing-xl)' }}>
                ...
                <div key={station.id}>
                  <div
                    className={`cursor-pointer transition-all duration-200 border ${
                      isComplete 
                        ...
                        : 'bg-white hover:scale-[1.005] active:scale-[0.995] border-black/5'
                    }`}
                    style={{ borderRadius: 'var(--radius-md)', boxShadow: 'var(--shadow-sm)', padding: 'var(--spacing-md)' }}
                  ...
                  >
                    <div className="flex items-start" style={{ gap: 'var(--spacing-md)' }}>
                      ...
                      <div className="flex-shrink-0 text-right min-w-[88px]">
                        {isStationLoading ? (
